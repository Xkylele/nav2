        ...
        while rclpy.ok():
            try:
                tf = self.buffer.lookup_transform(
                    'map', 'body_2d', rclpy.time.Time(seconds=0), rclpy.time.Duration(seconds=1))
...
def main():
    rclpy.init()
    patrol = PatrolNode()
    # 等待订阅者保证 第一次initialpose可以发布
    while patrol.initial_pose_publisher_.get_subscription_count() == 0:
        patrol.get_logger().info("Waiting for subscriber...")
        time.sleep(0.2)
